      ...
      <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
        ...
        <nav className="md:ml-auto md:mr-auto flex flex-wrap items-center text-base justify-center">
          {navigations.map((navigation) => (
            <Link
              to={navigation.path}
              className="mr-5 hover:text-pink-500 transition duration-300"
              key={navigation.name}
            >
              {navigation.name}
            </Link>
        ...
        <Link
          to={'/cart'}
          className="inline-flex items-center text-white bg-pink-500 border-0 py-2 px-4 focus:outline-none hover:bg-pink-700 rounded text-base mt-4 md:mt-0"
        >
          Go to Cart
          <svg
            fill="none"
